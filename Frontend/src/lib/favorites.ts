...
export function addFavorite(f: FavoriteRoute) {
  const all: FavoriteRoute[] = JSON.parse(localStorage.getItem(KEY) || "[]");
  if (all.some((x) => x.userId === f.userId && x.fromCode === f.fromCode && x.toCode === f.toCode)) return;
  all.push(f);
...
export function removeFavorite(userId: string, id: string) {
  const all: FavoriteRoute[] = JSON.parse(localStorage.getItem(KEY) || "[]");
  localStorage.setItem(KEY, JSON.stringify(all.filter((f) => !(f.userId === userId && f.id === id))));
}
