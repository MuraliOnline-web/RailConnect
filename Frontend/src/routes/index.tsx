...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Book suburban rail tickets, season passes and platform tickets in seconds. A premium, modern commuter experience." },
    ],
  }),
  component: Landing,
});
    ...
    <div className="relative min-h-screen overflow-x-clip bg-background">
      ...
      <section className="relative z-10 mx-auto grid max-w-7xl items-center gap-12 px-5 pt-10 pb-24 sm:px-8 md:grid-cols-[1.05fr_minmax(0,0.95fr)] md:gap-14 md:pt-16 lg:gap-24">
        ...
        >
          ...
          >
            RailConnect is a premium mobile-first ticketing platform for suburban
            rail commuters — paperless tickets, season passes and live journey
            planning, designed to feel effortless.
          </motion.p>
          <motion.div
            variants={{
              hidden: {},
          ...
          >
            ...
            <Link
              to="/login"
              className="cta cta-outline shadow-soft"
            >
          ...
          >
            <div className="flex items-center gap-2"><FaShieldHalved className="text-orange-500" /> Secure by design</div>
            <div className="flex items-center gap-2"><FaBoltLightning className="text-orange-500" /> Tickets in 10 s</div>
          </motion.div>
        </motion.div>
            ...
            <div className="ticket-lift glass premium-border relative overflow-hidden rounded-[2rem] p-1 shadow-glow">
              <div className="rounded-[1.75rem] bg-white p-5 sm:p-8 md:p-6 lg:p-9">
                <div className="flex items-baseline justify-between gap-3 text-[11px] sm:text-xs text-muted-foreground">
                  <span className="font-semibold uppercase leading-none tracking-[0.16em]">Mobile Ticket</span>
                  <span className="font-medium leading-none tabular-nums tracking-wide">#RC-228194</span>
                </div>
                <div className="mt-6 sm:mt-8 md:mt-8 lg:mt-9 grid grid-cols-[1fr_auto_1fr] items-center gap-x-2 sm:gap-x-3">
                  <div className="min-w-0">
                    <div className="mb-2 text-[11px] font-medium uppercase tracking-[0.18em] text-muted-foreground">From</div>
                    <div className="font-[Sora] text-[1.05rem] sm:text-[1.3rem] md:text-[1.1rem] lg:text-[1.3rem] font-bold leading-[1.15] tracking-tight">SOURCE</div>
                  </div>
                  ...
                  <div className="min-w-0 text-right">
                    <div className="mb-2 text-[11px] font-medium uppercase tracking-[0.18em] text-muted-foreground">To</div>
                    <div className="font-[Sora] text-[1.05rem] sm:text-[1.3rem] md:text-[1.1rem] lg:text-[1.3rem] font-bold leading-[1.15] tracking-tight">DESTINATION</div>
                  </div>
                </div>
                <div className="my-7 sm:my-8 md:my-8 lg:my-9 grid grid-cols-[1fr_auto_1fr] items-center gap-3">
                  ...
                  <span className="text-[10px] font-semibold uppercase tracking-[0.2em] text-orange-600 sm:text-[11px]">Western Line</span>
                ...
                <div className="grid grid-cols-3 items-start gap-2 sm:gap-3 text-[11px] sm:text-xs">
                  <div className="min-w-0 text-left">
                    <div className="text-[10px] font-medium uppercase tracking-[0.16em] text-muted-foreground sm:tracking-[0.18em]">Train Type</div>
                    <div className="mt-1 font-semibold text-foreground sm:text-[12.5px] md:text-[12px] lg:text-[12.5px]">Passenger/Express</div>
                  </div>
                  <div className="min-w-0 text-center">
                    <div className="text-[10px] font-medium uppercase tracking-[0.16em] text-muted-foreground sm:tracking-[0.18em]">Passengers</div>
                    <div className="mt-1 font-semibold text-foreground tabular-nums sm:text-[12.5px] md:text-[12px] lg:text-[12.5px]">1</div>
                  </div>
                  <div className="min-w-0 text-right">
                    <div className="text-[10px] font-medium uppercase tracking-[0.16em] text-muted-foreground sm:tracking-[0.18em]">Fare</div>
                    <div className="mt-1 font-semibold text-foreground tabular-nums sm:text-[12.5px] md:text-[12px] lg:text-[12.5px]">₹35.00</div>
                  </div>
                </div>
                <div className="mt-8 sm:mt-10 md:mt-9 lg:mt-11 rounded-2xl bg-orange-50 p-2.5 sm:p-4 text-center">
                  <div className="grid grid-cols-12 gap-[2px] sm:gap-[5px] md:gap-[4px] lg:gap-[5px]">
                    {Array.from({ length: 48 }).map((_, i) => (
                      <div key={i} className="h-7 sm:h-[28px] rounded-sm" style={{ background: i % 3 === 0 ? "#1a0e08" : i % 5 === 0 ? "#e8470e" : "#2a1810" }} />
                    ))}
                  </div>
                  <div className="mt-2.5 text-[10px] font-semibold uppercase tracking-[0.2em] text-orange-700 sm:mt-3.5">Scan at gate</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="grid items-center gap-8 md:grid-cols-3">
            <div className="md:col-span-2">
              ...
              <p className="mt-4 max-w-2xl text-muted-foreground">
                Inspired by the operational scale of UTS and RailConnect, redesigned
                ground-up with a calm, modern interface. No queues, no paper —
                just you, your platform, and the next train home.
              </p>
            </div>
      ...
      <section id="why" className="relative z-10 mx-auto max-w-7xl px-5 pb-28 sm:px-8">
        <div className="grid gap-5 md:grid-cols-3">
          {[
            { t: "Premium experience", d: "Calm typography, soft shadows and motion that feels like a luxury app." },
            { t: "Designed for India", d: "Suburban network-aware UI, season passes, platform tickets and live PNR." },
            { t: "Privacy first", d: "Your travel history stays yours. Encrypted, portable, deletable in one tap." },
          ].map((c, i) => (
            <motion.div
              key={c.t}
      ...
      <section className="relative z-10 mx-auto max-w-7xl px-5 pb-24 sm:px-8">
        <div className="bg-railway-gradient relative overflow-hidden rounded-3xl p-10 text-white shadow-glow sm:p-14">
          ...
          <div aria-hidden className="absolute -bottom-20 -left-10 h-72 w-72 rounded-full bg-black/10 blur-2xl" />
          <div className="relative grid items-center gap-6 md:grid-cols-2">
            <div>
              ...
              <p className="mt-3 max-w-md text-white/85">
                Create your free RailConnect account and get your first paperless
                ticket in under a minute.
              </p>
            </div>
            <div className="flex flex-wrap gap-3 md:justify-end">
              <Link
                to="/register"
                className="cta cta-light shadow-soft"
              >
                Create Account <FaArrowRight />
              </Link>
              <Link
                to="/login"
                className="cta cta-on-dark"
              >
                Already have Account <FaArrowRight />
              </Link>
            </div>
