...
function PlatformPage() {
  ...
  const fare = calcFare({ fromCode: station, toCode: station, type: "platform", classType: "2nd", adults: count, children: 0 }) * count;
  ...
  return (
    <DashboardShell>
      <div className="mb-6">
        ...
        <p className="mt-1 text-sm text-muted-foreground">Valid for 2 hours at the selected station.</p>
      </div>

      <div className="grid gap-5 lg:grid-cols-3">
        <div className="glass rounded-3xl p-6 shadow-soft lg:col-span-2">
          <div className="text-xs font-semibold uppercase tracking-wider text-foreground/70">Station</div>
          <select
            value={station}
          ...
          >
            {STATIONS.map((s) => (
              <option key={s.code} value={s.code}>{s.name} ({s.code}) · {s.line}</option>
            ))}
          </select>
          ...
          <div className="mt-5 flex items-center justify-between rounded-2xl bg-white/80 p-4">
            ...
            <div className="flex gap-2">
              <button onClick={() => setCount(Math.max(1, count - 1))} className="h-9 w-9 rounded-full bg-orange-50 text-orange-700">−</button>
              <button onClick={() => setCount(count + 1)} className="bg-railway-gradient h-9 w-9 rounded-full text-white">+</button>
            </div>
          </div>
        </div>
